    ...
    for symbol, splits in data.items():
        ...
        for split in splits:
            ...
            if split_time > period_begin:
                old_prices = stock_candle_db.query_candles(symbol, start_time=one_day_before.timestamp(), end_time=split_time.timestamp() - 1, candle_type="1m")
                new_prices = stock_candle_db.query_candles(symbol, start_time=split_time.timestamp(), end_time=one_day_after.timestamp(), candle_type="1m")
