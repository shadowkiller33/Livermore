...
class FinnhubEngine:
    ...
    def compute_stock_candles_of_a_resolution(self, one_minute_data, resolution="1H", working_hours_only=True):
        ...
        daytime_baseline = begin_time - timedelta(days=begin_time.weekday())  
        daytime_baseline = daytime_baseline.replace(hour=0, minute=0, second=0, microsecond=0)
        daytime_baseline = daytime_baseline.timestamp()
        
        day_seconds = 24 * 3600
        week_seconds = 7 * day_seconds
        
        begin_time = begin_time.timestamp()
        resolution = int(time_to_seconds(resolution))
        
        st = time.time()    
        ret = defaultdict(list)
        i = 0
        while i < len(time_slots) and begin_time > time_slots[i]:
            i +=1 
        while i < len(time_slots):
            end_time = begin_time + resolution
            # print(begin_time, i, pd.Timedelta(resolution))
            itme = defaultdict(list)
            while i < len(time_slots) and begin_time <= time_slots[i] < end_time:
                if working_hours_only:
                    week_time = (time_slots[i] - daytime_baseline) % week_seconds
                    day_time = (time_slots[i] - daytime_baseline) % day_seconds
                    # ny_slot = get_ny_time(time_slots[i])
                    # if ny_slot.weekday() >= 5:
                    #     continue
                        
                    if not (day_time > 9.5 * 3600 and day_time < 16 * 3600) or week_time > 5 * day_seconds:
                        i += 1
                        continue
